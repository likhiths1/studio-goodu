...
export default function Navbar() {
  ...
  return (
    <nav className="w-full bg-transparent font-inter z-20 px-4 md:px-12 py-2 flex items-center justify-between relative">
      ...
      >
        <span className={`block h-0.5 w-7 rounded-full my-1 transition bg-[#EDE1D5] ${open ? 'rotate-45 translate-y-2' : ''}`}></span>
        <span className={`block h-0.5 w-7 rounded-full my-1 transition bg-[#EDE1D5] ${open ? 'opacity-0' : ''}`}></span>
        <span className={`block h-0.5 w-7 rounded-full my-1 transition bg-[#EDE1D5] ${open ? '-rotate-45 -translate-y-2' : ''}`}></span>
      </button>
